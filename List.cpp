...
ll numDiv(ll N) {
  ll PF_idx = 0, PF = primes[PF_idx], ans = 0;             // start from ans = 1
  while (N != 1 && (PF * PF <= N)) {
    ll power = 0;                                             // count the power
    while (N % PF == 0) { N /= PF; power++; }
    ans += (power);                              // according to the formula
    PF = primes[++PF_idx];
  }
  if (N != 1) ans += 1;             // (last factor has pow = 1, we add 1 to it)
  return ans;
}
...
int main() {
  ...
  sieve(40000);                       // can go up to 10^7 (need few seconds)
  int X;
  scanf("%d",&X);
  printf("%lld",numDiv(X));
  return 0;
